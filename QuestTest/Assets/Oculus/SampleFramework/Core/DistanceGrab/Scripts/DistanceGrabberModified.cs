...
namespace OculusSampleFramework
{
    ...
    public class DistanceGrabberModified : OVRGrabber
    {
        ...
        [SerializeField]
        GameObject m_player;
        DistanceGrabberModified m_otherHand;
    ...
    void Update()
        ...
        protected override void GrabBegin()
        {
            ...
            if (closestGrabbable != null)
            {
                ...
                {
                    ((DistanceGrabberModified)closestGrabbable.grabbedBy).OffhandGrabbed(closestGrabbable);
                }
        ...
        protected override void MoveGrabbedObject(Vector3 pos, Quaternion rot, bool forceTeleport = false)
        {
            ...
            {
                Quaternion relOri = Quaternion.Inverse(m_otherHand.transform.rotation);// * m_grabbedObj.transform.rotation;
                m_grabbedObjectRotOff = relOri;
            }

            if (OVRInput.Get(OVRInput.RawButton.LIndexTrigger) && m_canScaleObjects)
            {
                ((DistanceGrabbableFloater)m_grabbedObj).Grow();
                m_canScaleObjects = false;
            }
            else if (OVRInput.Get(OVRInput.RawButton.RIndexTrigger) && m_canScaleObjects)
            {
